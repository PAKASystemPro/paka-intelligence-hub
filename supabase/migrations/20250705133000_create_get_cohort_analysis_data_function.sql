...
BEGIN
    ...
    cohort_summary AS (
        SELECT
            ...
            (SELECT COUNT(DISTINCT c.customer_id) FROM customer_cohorts c WHERE c.cohort_month = cc.cohort_month AND c.customer_id NOT IN (SELECT so2.customer_id FROM second_orders so2)) AS opportunity_count
    ...
    final_cohorts AS (
        SELECT
            ...
            jsonb_object_agg(
                'm' || EXTRACT(YEAR FROM cmd.second_order_month)::INT - EXTRACT(YEAR FROM cs.cohort_month)::INT * 12 + EXTRACT(MONTH FROM cmd.second_order_month)::INT - EXTRACT(MONTH FROM cs.cohort_month)::INT,
                jsonb_build_object(
                    'count', cmd.count,
            ...
            ) FILTER (WHERE cmd.count IS NOT NULL) AS monthly_data
        FROM cohort_summary cs
        LEFT JOIN cohort_monthly_data cmd ON cs.cohort_month = cmd.cohort_month
        GROUP BY cs.cohort_month, cs.new_customers, cs.total_second_orders, cs.opportunity_count
    ),
    grand_total_calc AS (
        SELECT
    ...
    SELECT json_build_object(
        'cohorts', COALESCE((SELECT json_agg(fc) FROM final_cohorts fc), '[]'::json),
