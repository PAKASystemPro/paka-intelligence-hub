...
// Reference data from Image 4 (ALL customers)
const referenceData = {
  ...
  try {
    const { data: customer, error } = await supabase
      ...
      .insert({
        ...
        total_spent: 0,
        orders_count: 0,
        // Note: created_at doesn't determine cohort month, but we'll set it to match the first order date
        created_at: new Date(`${month}-01T12:00:00Z`),
        updated_at: new Date(`${month}-01T12:00:00Z`)
      })
      .select()
      .single();
...
async function createOrder(customer, orderNumber, month, isSecondOrder = false) {
  ...
  // For first orders, use the beginning of the month (1st-10th)
  // For second orders, use later in the month (15th-25th)
  const dayOfMonth = isSecondOrder ? 
  ...
  try {
    const { data: order, error } = await supabase
      ...
      .insert({
        ...
        // Important: processed_at determines the cohort month for first orders
        processed_at: new Date(`${orderMonth}-${dayOfMonth.toString().padStart(2, '0')}T12:00:00Z`),
        updated_at: new Date()
      })
      .select()
      .single();
...
async function insertCorrectCohortData() {
  console.log('Starting insertion of correct cohort data for January-June 2025...');
  ...
  try {
    ...
    if (lineItemError) {
      console.error('Error deleting test line items:', lineItemError);
    } else {
      console.log('Test line items deleted successfully');
    }
    ...
    if (orderError) {
      console.error('Error deleting test orders:', orderError);
    } else {
    ...
    if (customerError) {
      console.error('Error deleting test customers:', customerError);
    } else {
      console.log('Test customers deleted successfully');
    }
  } catch (error) {
    console.error('Exception cleaning up test data:', error);
  }
  ...
  for (const month of months) {
    ...
    console.log(`Creating ${newCustomers} customers with ${secondOrders} second orders...`);
    ...
    for (let i = 1; i <= newCustomers; i++) {
      ...
      // Create first order - IMPORTANT: This sets the cohort month based on processed_at
      ...
      // We need exactly secondOrders customers with second orders
      if (i <= secondOrders) {
      ...
      if (i % 20 === 0) {
        ...
        console.log(`Progress: ${i}/${newCustomers} customers created for ${month}`);
      }
    }
    
    console.log(`Completed ${month}: ${newCustomers} customers with ${secondOrders} second orders`);
  }
  ...
  try {
    ...
    if (allCohortError) {
      ...
      // Calculate retention rates and grand totals
      Object.keys(cohortSummary).forEach(month => {
        ...
        grandTotal.newCustomers += newCustomers;
        grandTotal.secondOrders += secondOrders;
      });
  ...
  console.log('\nCorrect cohort data insertion completed successfully!');
}

// Run the main function
insertCorrectCohortData().catch(error => {
