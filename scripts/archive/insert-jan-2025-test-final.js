...
// Initialize Supabase client with service role key and explicitly set schema for table operations
...
const supabaseRpc = createClient(
  process.env.NEXT_PUBLIC_SUPABASE_URL,
  process.env.SUPABASE_SERVICE_ROLE_KEY
);
...
async function main() {
  try {
    ...
    for (const productCohort of productCohorts) {
      ...
      for (let i = 1; i <= 3; i++) {
        ...
        if (firstOrderError) {
          ...
          console.error('Full error details:', JSON.stringify(firstOrderError, null, 2));
          return;
        }
        ...
        console.log(`First order created with ID: ${firstOrder.id}`);
        ...
        if (firstLineItemError) {
          ...
          console.error('Full error details:', JSON.stringify(firstLineItemError, null, 2));
          return;
        }
        
        console.log(`First order line item created successfully`);
        ...
        if (hasSecondOrder) {
          ...
          if (secondOrderError) {
            ...
            console.error('Full error details:', JSON.stringify(secondOrderError, null, 2));
            return;
          }
          ...
          console.log(`Second order created with ID: ${secondOrder.id}`);
          ...
          if (secondLineItemError) {
            ...
            console.error('Full error details:', JSON.stringify(secondLineItemError, null, 2));
            return;
          }
          
          console.log(`Second order line item created successfully`);
        }
      }
    }
    ...
    // Use the RPC-specific client without schema setting
    const { data: classifyData, error: classifyError } = await supabaseRpc.rpc('classify_new_customers');
    ...
    // Use the RPC-specific client without schema setting
    const { data: refreshData, error: refreshError } = await supabaseRpc.rpc('refresh_materialized_views');
    ...
    // Use the RPC-specific client without schema setting
    const { data: allHeatmap, error: allHeatmapError } = await supabaseRpc.rpc('get_cohort_heatmap');
    
    if (allHeatmapError) {
      console.error('Error getting ALL cohort heatmap:', allHeatmapError);
      console.error('Full error details:', JSON.stringify(allHeatmapError, null, 2));
    } else {
      console.log('\nALL Cohort Heatmap:');
    ...
    const { data: ssHeatmap, error: ssHeatmapError } = await supabaseRpc.rpc('get_cohort_heatmap_by_product', { 
      p_product_cohort: '深睡寶寶' 
    });
    
    if (ssHeatmapError) {
      console.error('Error getting 深睡寶寶 cohort heatmap:', ssHeatmapError);
      console.error('Full error details:', JSON.stringify(ssHeatmapError, null, 2));
    } else {
      console.log('\n深睡寶寶 Cohort Heatmap:');
    ...
    const { data: twHeatmap, error: twHeatmapError } = await supabaseRpc.rpc('get_cohort_heatmap_by_product', { 
      p_product_cohort: '天皇丸' 
    });
    
    if (twHeatmapError) {
      console.error('Error getting 天皇丸 cohort heatmap:', twHeatmapError);
      console.error('Full error details:', JSON.stringify(twHeatmapError, null, 2));
    } else {
      console.log('\n天皇丸 Cohort Heatmap:');
    ...
    const { data: hhHeatmap, error: hhHeatmapError } = await supabaseRpc.rpc('get_cohort_heatmap_by_product', { 
      p_product_cohort: '皇后丸' 
    });
    
    if (hhHeatmapError) {
      console.error('Error getting 皇后丸 cohort heatmap:', hhHeatmapError);
      console.error('Full error details:', JSON.stringify(hhHeatmapError, null, 2));
    } else {
      console.log('\n皇后丸 Cohort Heatmap:');
  ...
  try {
    ...
    try {
      ...
      if (lineItemsError) {
        ...
        console.error('Full error details:', JSON.stringify(lineItemsError, null, 2));
      } else {
      ...
      if (customersError) {
        ...
        console.error('Full error details:', JSON.stringify(customersError, null, 2));
      } else {
