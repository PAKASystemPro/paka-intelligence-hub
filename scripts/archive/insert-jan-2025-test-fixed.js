  ...
  try {
    ...
    for (const productCohort of productCohorts) {
      ...
      for (let i = 1; i <= 3; i++) {
        ...
        // Insert customer using RPC function
        ...
        // The customer.id is a UUID, but insert_test_order expects an integer
        // We need to use a numeric ID for the customer
        const customerNumericId = i; // Use the loop index as a simple numeric ID
        ...
        if (hasSecondOrder) {
          ...
          const firstOrderDate = new Date(createdAt);
  ...
  try {
    ...
    try {
      ...
      if (lineItemsError) {
        ...
        console.log('Will try direct table access...');
        
        // Try direct table access with schema
        const { error: directLineItemsError } = await supabase
          .from('order_line_items')
          .delete()
          .neq('id', 0);
          
        if (directLineItemsError) {
          console.error('Error deleting line items directly:', directLineItemsError);
        } else {
          console.log('Cleared line items via direct table access');
        }
      } else {
      ...
      if (ordersError) {
        ...
        // Try direct table access with schema
        const { error: directOrdersError } = await supabase
          .from('orders')
          .delete()
          .neq('id', 0);
          
        if (directOrdersError) {
          console.error('Error deleting orders directly:', directOrdersError);
        } else {
          console.log('Cleared orders via direct table access');
        }
      } else {
      ...
      if (customersError) {
        ...
        // Try direct table access with schema
        const { error: directCustomersError } = await supabase
          .from('customers')
          .delete()
          .neq('id', 0);
          
        if (directCustomersError) {
          console.error('Error deleting customers directly:', directCustomersError);
        } else {
          console.log('Cleared customers via direct table access');
        }
      } else {
