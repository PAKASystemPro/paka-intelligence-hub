...
async function calculateNewCustomersStrict() {
  try {
    ...
    for (const customer of customers) {
        customerCreatedAt[customer.id] = new Date(customer.created_at);
    }
    ...
    for (const customerId in customerFirstOrder) {
        if (customerCreatedAt[customerId]) {
            const createdAt = customerCreatedAt[customerId];
    ...
    console.log('\n## New Customer Count Verification (Strict Logic)');
    console.log('| Cohort Month | Calculated New Customers | Reference New Customers | Difference | % Difference |');
    console.log('|--------------|--------------------------|-------------------------|------------|--------------|');
    ...
    for (const month of sortedMonths) {
        ...
        const percentageDifference = ((difference / reference) * 100).toFixed(2);
        console.log(`| ${month}      | ${String(calculated).padEnd(24)} | ${String(reference).padEnd(23)} | ${String(difference).padEnd(10)} | ${String(percentageDifference + '%').padEnd(12)} |`);
    }
