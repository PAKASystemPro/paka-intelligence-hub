...
async function verifyData() {
  console.log(`🚀 Verifying January 2025 data for ${startDate} to ${endDate}...`);
  ...
  try {
    ...
    // 5. Get Cohort Verification Stats
    console.log('📊 Verifying Cohort Metrics...');
    const { data: cohortData, error: cohortError } = await supabase.rpc('public.get_cohort_verification_stats', {
    ...
    const newCustomers = cohortData.find(d => d.metric === 'new_customers')?.value || 0;
    const secondOrderCustomers = cohortData.find(d => d.metric === 'second_order_customers')?.value || 0;

    console.log(`✅ New Customers: ${newCustomers} found.`);
    console.log(`✅ 2nd Order Customers: ${secondOrderCustomers} found.`);
